...
impl DownloadClient for SabnzbdDownloadClient {
    async fn discover_categories(&self, _client_id: &str) -> AppResult<Option<Vec<String>>> {
        ...
        let categories = value.get("categories").cloned().ok_or_else(|| {
            AppError::Repository("SABnzbd category response is missing categories".into())
        })?;
        let names: Vec<String> = serde_json::from_value(categories).map_err(|error| {
            AppError::Repository(format!("invalid SABnzbd categories: {error}"))
        })?;
        Ok(Some(names))
    }
