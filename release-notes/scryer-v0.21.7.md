# Scryer 0.21.7 release notes

These notes cover what's changed since **0.21.4**. Scryer 0.21.5 and 0.21.6 were tagged but never published; this release includes all of their changes.
...
- **Import:** the same repeated-copy problem could also happen with extra files that came with an episode download. A repeat import of an extra file Scryer already copied is now recognised and skipped. A finished download that Scryer can't yet confirm is fully imported is now checked again after a growing delay (30 seconds, then 2, 5 and 15 minutes) instead of being imported again on every check.
