...
- **Scryer uses far less memory.** Scryer now manages its own memory rather than leaving it to the system, on every platform. On a test library of about 111,000 items, memory use used to climb to roughly 3 GB and never level off; it now settles at about 500 MB and stays there. Releasing unused memory was also moved onto its own background threads, which brought average memory during the same test down from 687 MB to 532 MB and Scryer's CPU usage. Nothing needs configuring. The ARM64 Linux image is built to start on kernels with 4 KiB, 16 KiB or 64 KiB memory pages, so it runs on systems such as the Raspberry Pi 5 and Asahi Linux as well as ordinary ARM64 servers.
...
- **Indexer rate limits:** when an indexer answers "too many requests", Scryer now treats it as a cooldown rather than a failure. It waits as long as the indexer asks, up to 24 hours, and when the indexer gives no wait time each repeat lengthens the pause, which only resets once the indexer answers normally again. Background searches are also spread out to one request every two seconds per indexer, so a large wanted list no longer arrives as a burst. An indexer that is cooling down is shown as cooling down, not as an error.
- **Downloads:** a release Scryer sent to a download client that the client then never listed — removed at once, or rejected after accepting it — stayed marked as in progress, so Scryer never searched for that item again. After a minute without the client listing it, Scryer now treats the download as gone and the item goes back to being wanted.
- **Downloads:** after you deleted a download, searching for the same item within about 30 seconds could refuse the new grab because Scryer still counted the deleted download as in progress. Deleting a download now clears that straight away.
- **Downloads:** a torrent that Scryer imported and then left in the client to keep seeding was no longer watched, so when you later removed it from the client Scryer never noticed and kept holding on to it. Scryer now keeps watching torrents it leaves in the client after import and lets go of them once you remove them. Usenet downloads are unaffected.
- **Episodes:** episodes of the same season could share a single wanted entry, so grabbing or searching for one episode could change what Scryer recorded for another, and an RSS release for an episode was occasionally missed. Each episode now has its own wanted entry.
- **Indexers:** saving an indexer no longer contacts it again when you have only changed local settings such as its name or priority. It is contacted only when its address, API key, proxy or connection settings change. The capabilities an indexer reports are kept for seven days instead of being fetched again on every save and test, which spares indexers with tight request limits.
- **Indexers:** a reply to a capabilities request that is empty, cut off, or not a capabilities document — an HTML error page, for example — is now reported as an error instead of being accepted as an indexer with no categories. A capabilities check that finishes late can no longer overwrite the indexer's more recent health status.
- **Title pages:** a file imported in the first moments after you opened a title's page could stay missing from the page until you navigated away and back. The page now catches up as soon as it starts listening for updates.
- **Artwork (PostgreSQL):** two pages loading the same artwork at the same moment could fail with a database deadlock and leave the page without its content. Artwork is now registered in a fixed order, so this can no longer happen.
...
Three changes are made on first start: the multilingual title spelling index is built, a new index is added for the dashboard, and a wanted-list index is narrowed so each episode keeps its own wanted entry. On a large library the first start can take a little longer than usual while the title index is built. PostgreSQL installations build the same index when Scryer starts.
